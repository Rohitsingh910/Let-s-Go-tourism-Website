// Check the current page and apply the active class
$(document).ready(function () {
    function removeall() {
        $(".cir_border").css("border", "none");
    }

    // Check if we are on 'destinations.html'
    if (window.location.pathname.indexOf("destinations.html") > -1) {
        // Apply border to Destinations link when on destinations page
        removeall();
        $("#tri").css("border", "2px solid whitesmoke");
        $("#tri").css("border-radius", "20px");
    }

    // Handle click events on each navigation item
